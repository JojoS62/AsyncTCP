...
#include "esp_log.h"
static const char* TAG = "AsyncPrinter";
  ...
  , next(NULL)
{
  ...
  if(_tx_buffer == NULL) {
    esp_system_abort("no memory for AsyncPrinter buffer");
...
void AsyncPrinter::onConnect(ApCloseHandler cb, void *arg){
  _connect_cb = cb;
  _connect_cb_arg = arg;
}
...
int AsyncPrinter::connect(IPAddress ip, uint16_t port){
  ...
  if (_client == NULL) {
    esp_system_abort("client is NULL");
    //panic();
  }
...
int AsyncPrinter::connect(const char *host, uint16_t port){
  ESP_LOGI(TAG , "client connecting");
...
AsyncPrinter & AsyncPrinter::operator=(const AsyncPrinter &other){
  ...
  if(_tx_buffer == NULL) {
    esp_system_abort("_tx_buffer is NULL");
    //panic();
  }
...
size_t AsyncPrinter::_sendBuffer(){
  ...
  if (out == NULL) {
    ...
    esp_system_abort("no out buffer");
  }
